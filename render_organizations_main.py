...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-map-pin-house"><path d="M15 22a1 1 0 0 1-1-1v-4a1 1 0 0 1 .445-.832l3-2a1 1 0 0 1 1.11 0l3 2A1 1 0 0 1 22 17v4a1 1 0 0 1-1 1z"/><path d="M18 10a8 8 0 0 0-16 0c0 4.993 5.539 10.193 7.399 11.799a1 1 0 0 0 .601.2"/><path d="M18 22v-3"/><circle cx="10" cy="10" r="3"/></svg>
    ...
    if html != '' or input_data['identity'][0]['llm_location'] != '':
        html = f'''
            <section>
                <h2>Location</h2>
    ...
    if html_plants != '':
        html = f'''
            <section style="margin-bottom: 4.8rem;">
                ...
                <ul style="list-style: none; display: flex; flex-wrap: wrap; gap: 1.6rem;">
                    {html_plants}
                </ul>
            </section>
        '''
...
def section_activities_gen(input_data):
    ...
    if input_data['identity'][0]['llm_activities'] != '':
        html = f'''
            <section style="margin-bottom: 4.8rem;">
...
def section_products_gen(input_data):
    ...
    if input_data['identity'][0]['llm_products'] != '':
        html = f'''
            <section>
                <h2>Products</h2>
...
def section_services_gen(input_data):
    ...
    ###
    if input_data['identity'][0]['llm_services']:
        html = f'''
            <section>
                <h2>Services</h2>
...
def render_listing(master_item):
    ...
    # if url_slug != 'organizations/acid-2-alkaline-herbal-market': return
    # print(json.dumps(input_data, indent=4))
    # return
    # quit()
    ...
    if identity_website_item['fields']['business_website_html'] == 'false': return
    if identity_website_item['fields']['business_website_text'] == 'false': return
    # kprint(url_slug)
    # return 1
    ###
    ...
    if not found:
        return
    ### SKIP MISSING IDENTITY WEBSITE DESCRIPTION
    for lst in input_data['identity']:
        for item in lst['items']:
            if item['source_name'] == 'Website':
                print(business_slug)
                if business_slug == 'the-herb-shop-anniston-al':
                    print(json.dumps(input_data, indent=4))
                    quit()
                if item['fields']['business_description'] == None: return
    '''
    # return
    ...
    ################################################################################
    html_article = ''
    if identity_gmap_item['fields']['business_rating']:
        business_rating = identity_gmap_item['fields']['business_rating'].replace(',', '.')
    else:
        business_rating = 'N/A'
    if identity_gmap_item['fields']['business_reviews_num']:
        business_reviews_num = identity_gmap_item['fields']['business_reviews_num'].replace('(', '').replace(')', '')
    else:
    ...
    hero_html = f'''
        ...
        <div style="display: flex; justify-content: space-between;">
            ...
            <div style="display: flex; flex-direction: column; align-items: center; margin-top: 0.8rem;">
                <span style="display: inline-block;" class="hero-score">{business_rating} / 5</span>
                <span style="display: inline-block;" class="hero-reviews-num">{business_reviews_num} Ratings</span>
            </div>
        </div>
    '''
    html_article += f''
    ...
    for master_item in master_items[:]:
        ...
        for identity_list in identity_data:
            for identity_item in identity_list['items']:
                if identity_item['source_name'] == 'Google Maps':
                    identity_gmap_item = identity_item
        # print(json.dumps(identity_gmap_item, indent=4))
        # print('here')
        # quit()
        # continue
