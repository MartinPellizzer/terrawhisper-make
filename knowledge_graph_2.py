...
vertices_plants_filepath = f'{vault}/herbalism/vertices-plants.json'
if not os.path.exists(vertices_plants_filepath): 
    json_write(vertices_plants_filepath, [])
vertices_plants = json_read(vertices_plants_filepath)

vertices_families_filepath = f'{vault}/herbalism/vertices-families.json'
if not os.path.exists(vertices_families_filepath): 
    json_write(vertices_families_filepath, [])
vertices_families = json_read(vertices_families_filepath)

vertices_orders_filepath = f'{vault}/herbalism/vertices-orders.json'
if not os.path.exists(vertices_orders_filepath): 
    json_write(vertices_orders_filepath, [])
vertices_orders = json_read(vertices_orders_filepath)

vertices_subclasses_filepath = f'{vault}/herbalism/vertices-subclasses.json'
if not os.path.exists(vertices_subclasses_filepath): 
    json_write(vertices_subclasses_filepath, [])
vertices_subclasses = json_read(vertices_subclasses_filepath)

vertices_classes_filepath = f'{vault}/herbalism/vertices-classes.json'
if not os.path.exists(vertices_classes_filepath): 
    json_write(vertices_classes_filepath, [])
vertices_classes = json_read(vertices_classes_filepath)

vertices_divisions_filepath = f'{vault}/herbalism/vertices-divisions.json'
if not os.path.exists(vertices_divisions_filepath): 
    json_write(vertices_divisions_filepath, [])
vertices_divisions = json_read(vertices_divisions_filepath)

edges_families_orders_filepath = f'{vault}/herbalism/edges-families-orders.json'
if not os.path.exists(edges_families_orders_filepath): 
    json_write(edges_families_orders_filepath, [])
edges_families_orders = json_read(edges_families_orders_filepath)

edges_orders_subclasses_filepath = f'{vault}/herbalism/edges-orders-subclasses.json'
if not os.path.exists(edges_orders_subclasses_filepath): 
    json_write(edges_orders_subclasses_filepath, [])
edges_orders_subclasses = json_read(edges_orders_subclasses_filepath)

edges_subclasses_classes_filepath = f'{vault}/herbalism/edges-subclasses-classes.json'
if not os.path.exists(edges_subclasses_classes_filepath): 
    json_write(edges_subclasses_classes_filepath, [])
edges_subclasses_classes = json_read(edges_subclasses_classes_filepath)

edges_classes_divisions_filepath = f'{vault}/herbalism/edges-classes-divisions.json'
if not os.path.exists(edges_classes_divisions_filepath): 
    json_write(edges_classes_divisions_filepath, [])
...
if 0:
    plants_wcvp = csv_read_rows_to_json(f'{vault}/herbalism/wcvp_taxon.csv', delimiter = '|')
        ...
        if family not in edges_families: 
            ...
            for _ in range(1):
                ...
                if json_reply != {}:
                    ...
                    if len(plant_kingdom.split(' ')) > 1: continue
                    if len(plant_division.split(' ')) > 1: continue
                    if len(plant_class.split(' ')) > 1: continue
                    if len(plant_subclass.split(' ')) > 1: continue
                    if len(plant_order.split(' ')) > 1: continue
                    if len(plant_family.split(' ')) > 1: continue
                    if plant_kingdom == 'none' or plant_kingdom == 'n/a': continue
                    if plant_division == 'none' or plant_division == 'n/a': continue
                    if plant_class == 'none' or plant_class == 'n/a': continue
                    if plant_subclass == 'none' or plant_subclass == 'n/a': continue
                    if plant_order == 'none' or plant_order == 'n/a': continue
                    if plant_family == 'none' or plant_family == 'n/a': continue
                    print('***************************************')
                    edge_new = {
